...
void password_page()
{
    ...
    printf("           ||  |--.STORY|   ||||   |  |===+===|   | |__|  |   |  |   .    /      |  \n");
    printf("           ||==|~~|==+==|   ||||   |  | BOOKS |===| |==|  |   |  |    '  /       |  \n");
    printf("           ||  |__|     |LMS|  |   |__|~~~~~~~|   |_|__|  |~~~|__|     )(        |  \n");
    printf("           ||  |  |_____|===|--|===|--|       |===|~|--|  |~~~|--|   _'  '_      |  \n");
    printf("           ||__|__|_____|___|__|___|__|_______|___|_|__|__|___|__|_.'______'.____|\n\n");
    printf("           .---------------------------------------------------------------------.\n");
    printf("           |                              MainPage PAGE                             |\n");
    printf("           '---------------------------------------------------------------------'\n");
    password();
}
void password()
{
    int n = 0;
    char user_name[50] = {"libarian"};
    char pass_word[50] = {"acme"};
    char user[50];
    char pass[50];
    printf("\nEnter your username:");
    scanf("%s", &user);
...
void deletebook()
{
    ...
    while (fscanf(f, "%s\t\t%s\t\t%s\t\t%d\n", b.book_name, b.author_name, b.book_category, &b.book_id) != EOF)
    {
        if (b.book_id != id)//yo id namileko data new file ma store garne
        {
